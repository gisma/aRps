...
makenc3D<-function(file,varname){
  ...
  if (refInfo$proj=="+proj=longlat +datum=WGS84 +no_defs"){
    ...
    nc.x <- ncdim_def( "lon", "degrees_east", refInfo$coordx[1:orig.nc$dim$x$len],create_dimvar=TRUE)
    nc.y <- ncdim_def( "lat", "degrees_north",refInfo$coordy[1:orig.nc$dim$y$len],create_dimvar=TRUE)
    ...
    nc.z <- ncdim_def( "level", "m", orig.nc$var$ZP$dim[[3]][[9]][1:orig.nc$dim$z$len],create_dimvar=TRUE)
  ...
  var_qs <- ncvar_def('SMR',longname='mass_fraction_of_snow_in_air','kg/kg', list(nc.x,nc.y,nc.z,nc.t), 1.e30 )
  ...
  ncvar_put( ncnew, var_zp, ncvar_get(orig.nc, 'V'))
  ncvar_put( ncnew, var_zp, ncvar_get(orig.nc, 'W'))
